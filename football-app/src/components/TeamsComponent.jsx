	...
	return (
		<div className='hover:bg-slate-200/40 group relative'>
			<div className='border-2 border-black shadow-xl rounded-md flex flex-col items-center p-2 h-60'>
				...
				<h1 className=' text-lg font-bold text-center mb-2'>{data?.result[0]?.team_name}</h1>
				<img className='' src={data?.result[0]?.team_logo} alt='/' />
					...
					<div
						...
						onClick={infoHandler}>
						{!info ? (<div className='mt-24 text-md font-bold text-white'>Click For More Info!</div>) : (
					<div onClick={infoHandler} className='mx-auto text-white font bold text-md flex  flex-col justify-center'>
						<span>Coach:</span>
						<span className=''>{data?.result[0]?.coaches[0]?.coach_name}</span>
